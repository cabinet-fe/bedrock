...
| 概念           | 说明                                                                                                                                 |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 用户           | 登录主体；可禁用；绑定多个角色；有效功能权限为各角色绑定 `full_code` 的并集（剔除 `super_admin_only`）                              |
| 超级管理员     | 鉴权真源为 `users.is_super_admin`；内置角色 `code=super_admin`（`type=builtin`）与唯一超管用户 1:1 同步；不可删、不可改权限、不可通过用户绑角 API 赋给他人；恒持有全部功能权限 |
| 内置业务角色   | 注册可选：开发 `developer` / 测试 `tester` / 运维 `ops` / 实施 `implementer` / 产品 `product`；`type=builtin`、`data_scope=self`；名称与编码锁定，权限矩阵、描述与数据范围可由管理员调整；不可删除 |
| 自定义角色     | `type=custom`；管理员可创建/编辑/删除；名称、编码、描述、`data_scope` 与功能权限均可编辑                                             |
...
- `super_admin_only`（当前仅仪表盘「系统信息」「系统状态」卡片，种子每次启动强制回写）：服务端拒绝绑入角色权限；生效时非超管一律拒绝。
- 产品项目内权限与全局 RBAC **同时生效**：见 DESIGN §4.4（`view_all` / `manage_all` + 项目成员角色）。

### 4.3 全局权限矩阵（首期基线）

| 能力                                 | 超级管理员 | 开发 | 测试 | 运维 | 实施 | 产品 | 自定义角色 | 无权限用户 |
| ------------------------------------ | ---------- | ---- | ---- | ---- | ---- | ---- | ---------- | ---------- |
| 运维（进程/开发环境）                | ✓          | ✗    | ✗    | ✓    | ✗    | ✗    | 按资源     | ✗          |
| 系统管理（用户/角色/资源/字典/日志） | ✓          | ✗    | ✗    | 仅备份/日志查看 | ✗ | ✗ | 按资源 | ✗ |
| 资源管理（仓库/服务器/凭证/令牌）     | ✓          | 仓库增改查、服务器查、凭证查+用、令牌全量 | 仓库/服务器查、令牌全量 | 服务器全量、凭证全量+用、仓库查、令牌全量 | 仓库/服务器查、凭证查+用、令牌全量 | 令牌全量 | 按资源 | ✗ |
| CI/CD                                | ✓          | 三类任务全量+执行 | 任务查看+触发（构建/脚本）、其余查看 | 同开发 | 全部只读 | ✗ | 按资源 | ✗ |
| 项目管理                             | ✓          | 项目增改查、工作项/文档全量 | 项目查、缺陷全量、需求增改查 | 项目查、缺陷增查、开发文档增改查 | 项目查、缺陷增改查、开发文档增改查 | 项目增改查、需求全量、缺陷增改查 | 按资源 + 项目成员 | ✗ |
| AI / Skills                          | ✓          | 智能体全量+执行、会话、技能全量+下载、服务商查 | 智能体查看+执行、技能下载 | ✗ | 技能下载 | 智能体查看+执行、技能下载 | 按资源 | ✗ |
| 仪表盘卡片                           | ✓（含系统信息/状态） | 业务卡片 | 业务卡片 | 业务卡片 | 业务卡片 | 我的项目+智能体运行 | 仅可见有资源权限的卡片 | 仅公共只读卡片（若有） |

内置角色数据权限一律 `data_scope=self`：数据默认仅自己可见；产品项目中作为项目成员可见项目本身，工作项（缺陷/需求/任务）中仅作为创建人或经办人可见他人条目（owner/admin 项目角色与全数据权限可见全部）；文档等其它协作内容仍按项目成员可见。不给内置角色 `view_all` / `manage_all`，跨数据范围能力留给自定义角色按需勾选。
...
- **自助注册**：`POST /auth/register` 公开端点（注册即登录，响应同登录）；受 `auth.allow_register` 配置控制（默认开）。注册时必选内置角色（`role_code`：`developer` / `tester` / `ops` / `implementer` / `product`），新用户绑定所选角色；数据权限均为 self，更多权限由管理员调整角色或换绑。
...
- 终态列（已关闭/已拒绝等）**常驻看板**并带「终态」标记，卡片数据一次拉全；接口层保留 `include_terminal` 默认排除的语义供插件/PAT 使用。
- 列头提供**快捷新建**按钮（需创建权限），点按按该列状态预选打开新建表单。
- 拖拽卡片到目标列即发起状态流转（受编辑权限约束，只读角色禁拖）；流转写活动记录。
- 缺陷工作台支持跨项目看板（卡片带所属项目）；项目详情内需求/缺陷面板为项目内看板。卡片均遵循工作项条目级读范围（协作者仅见自己创建或被指派的）。
