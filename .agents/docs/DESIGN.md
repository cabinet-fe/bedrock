...
| D7 | AI CLI | OpenCode / Reasonix 并行交付；Claude Code / Codex 支持已移除（harness 会话底座取代 CLI 执行路径） |
...
| D17 | PAT scope | 固定白名单：`skills:read`、`agents:run`、`docs:read`、`docs:write`、`dev_docs:read`、`dev_docs:write`、`builds:run`、`pipelines:run`、`scripts:run`、`bugs:read`、`bugs:write`；前缀 `br_`+hex（不兼容旧 `br_pat_`）；SHA-256 哈希鉴权 + AES-GCM 密文（属主 `GET .../reveal` 返回密文，前端用与登录相同的 `encryption.key` 解密后复制）、可过期/吊销；**不替代 HTTPS/TLS**；供 Skill 安装器、`agents:run`、构建/流水线/脚本执行、接口/开发文档与缺陷（浏览器插件 / bedrock 技能）读写开放 API 对接 |
...
4a. BYOK 提供商注入：启用的「AI 服务商/模型」渲染为各会话目录的 `opencode.json`（0600，提供商 id `bedrock-p{providerID}`）。每个 `bedrock-p*` 的 `options.baseURL` 指向本机 `http://127.0.0.1:{port}/api/v1/ai`（经 `ChatProxy` 按 model 转发上游），`options.apiKey` 为进程 loopback token（上游 Key 不落工作区）。模型目录与保存校验仅认 `bedrock-p*`。智能体默认推理强度（`ai_agents.reasoning_effort`）渲染为该目录内所选模型的 `options.reasoningEffort`。智能体字段 `inject_default_prompt`（默认 true）控制是否注入工作区/.env 默认提示词（编译进 `bedrock-*.md` 与 Run 提示词；仅中文）。
