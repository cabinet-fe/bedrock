...
function summarizeAuth(hits) {
  if (!hits || !hits.length) return "需要登录（源码未标注权限码）";
...
function findClassDeclaration(src) {
  const re =
    /\b((?:public|protected|private)\s+)?((?:abstract|final)\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)/g;
  let m;
  while ((m = re.exec(src))) {
  ...
  while ((m = methodRe.exec(src))) {
    ...
    for (const httpMethod of methods) {
      endpoints.push({
        ...
        requiresAuth: !effectiveAuth.some((h) => h.kind === "AnonymousAccess"),
        params,
...
function listEndpoints(srcRoot, opts = {}) {
  ...
  const controllers = [...byController.values()].sort((a, b) => a.docFile.localeCompare(b.docFile));
