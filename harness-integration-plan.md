...
- **`.opencode/agents/`（复数）路径（已确认可用）**：单复数均可（内置技能文档表：`.opencode/agent/<name>.md` 或 `.opencode/agents/<name>.md`，全局 `~/.config/opencode/agent(s)/`）。
- **技能目录再修正（2026-09-16，v1.18.31 二进制内嵌加载器反混淆核实 + TUI 实测，推翻上条 M0 的「项目级不发现」结论）**：加载器从会话目录**向上逐层扫描至 worktree**，对 `.claude/`、`.agents/` 目录用递归 glob `skills/**/SKILL.md`（仅复数；单数 `skill/` 仅 `.opencode/` 支持），**项目级 `.agents/skills/` 支持成立**——此前结论系探测方式缺陷（目录 API 的项目实例按 worktree 惰性加载并缓存，`/tmp` 等无 git 根目录落 global 项目，`/api/skill` 恒返回空，不代表真实发现）。该向上扫描对 bedrock「会话目录 ≠ 项目根」的布局反而比 `.opencode/` 更稳（后者只扫项目根 config 目录）。技能同步目标定为 `{agentWorkspace}/.agents/skills/<name>/`，同步时清除旧 `.opencode/skills/` 残留；v4 §3.3 的 `.agents/skills/` 假设恢复有效。
- **补充实测**（契约适配已按此实现）：
