...
function handleLinkNavigation(url: string, rowData?: Record<string, any>) {
  ...
  if (projectMatch && projectMatch[1]) {
    ...
    const projectName = rowData?.name || `项目 #${projectId}`;
  ...
  if (docMatch && docMatch[2]) {
    ...
    const isDev = !!docMatch[1];
    chatStore.openRightPanel({
      type: "doc",
      id: docId,
      projectId: rowData?.project_id ? Number(rowData.project_id) : undefined,
      docType: isDev ? "dev" : "api",
      title: rowData?.name ? `文档 · ${rowData.name}` : `文档 #${docId}`,
    });
    return;
  }
