...
import { Terminal, Queue, Refresh, CircleCheck } from "@veltra/icons/normal";
    ...
    <u-card-header class="tile__header">
      <div class="tile__title-row">
        <span class="tile__icon" aria-hidden="true">
          <u-icon :size="18" color="primary"><Terminal /></u-icon>
        </span>
        <div class="tile__titles">
          <h3 class="tile__title">脚本运行摘要</h3>
          <p class="tile__subtitle">近期脚本执行吞吐与结果</p>
        </div>
      </div>
    </u-card-header>
    ...
    <u-card-content class="tile__body">
      ...
      <div class="recent">
        <div class="recent__head">
          <span class="recent__title">近期执行</span>
        </div>
        <ul v-if="data?.recent?.length" class="recent__list">
          <li v-for="run in data.recent" :key="run.id">
            <button type="button" class="recent__row" @click="openRun(run)">
              <span class="recent__name">{{ run.job_name || `#${run.run_number}` }}</span>
        ...
        <p v-else class="recent__empty">暂无近期脚本执行记录</p>
      </div>
    </u-card-content>
  </u-card>
</template>
