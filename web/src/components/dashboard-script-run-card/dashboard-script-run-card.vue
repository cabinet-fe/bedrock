...
defineOptions({ name: "DashboardScriptRunCard" });

import { ArrowRight, CircleCheck, Queue, Refresh, Terminal } from "@veltra/icons/normal";

import type { DashboardRecentScriptRun, ScriptRunSummary } from "@/api/types";
import { formatDateTime } from "@/lib/datetime";
import { JOB_STATUS_TAG, jobStatusLabel, tagType } from "@/lib/tag";

const props = defineProps<{
  data: ScriptRunSummary | null;
}>();
...
function openRun(run: DashboardRecentScriptRun) {
  emit("openRun", run.id);
}
</script>
    ...
    <u-card-header class="tile__header">
      <div class="tile__title-row">
        <span class="tile__icon" aria-hidden="true">
          <u-icon :size="16" color="primary"><Terminal /></u-icon>
        </span>
        <div class="tile__titles">
          <h3 class="tile__title">脚本运行摘要</h3>
        </div>
        <button
          v-if="data?.running"
          type="button"
        ...
        <button type="button" class="tile__link-btn" title="查看脚本任务" @click="emit('openJobs')">
          <span>管理任务</span>
        ...
        <ul v-if="data?.recent?.length" class="task-list">
          <li v-for="run in data.recent" :key="run.id">
            <button type="button" class="task-row" @click="openRun(run)">
              <span class="task-row__name" :title="run.job_name || `#${run.run_number}`">
                {{ run.job_name || `#${run.run_number}` }}
              </span>
