...
- scope 白名单（创建时多选）：`skills:read`、`agents:run`、`docs:read`、`docs:write`、`dev_docs:read`、`dev_docs:write`、`builds:run`、`pipelines:run`、`scripts:run`、`bugs:read`、`bugs:write`。每个 scope 映射固定的开放端点（见「3. 开放接口一览」），scope 不足返回 `403 token scope insufficient`。
...
| `bugs:read`      | GET        | `/projects`、`/projects/bugs`、`/projects/{id}/bugs`、`/projects/{id}/bugs/{bugID}`（及活动 / 评论 / 附件列表与附件下载）                      | 项目列表（PAT 响应精简为 id / name / slug，仅本人可见项目）与缺陷读路径；缺陷端点另受项目 ACL 约束 |
| `bugs:write`     | POST / PUT | `/projects/{id}/bugs`、`/projects/{id}/bugs/{bugID}/status`、`/projects/{id}/bugs/{bugID}/comments`、`/projects/{id}/bugs/{bugID}/attachments` | 创建缺陷、流转状态、发表评论、上传附件（≤20MB）；另受项目 ACL 约束                                 |

缺陷接口的完整端点、查询参数（`assignee` 用户名或用户 ID、`exclude_closed` 未关闭口径）与响应结构以契约 `api/project.md`「缺陷」为准。

以上接口也接受登录 JWT（此时校验 RBAC 权限而非 scope）：`api-runs` 需 `ai_agents:execute`，构建入队需 `cicd_build_jobs:execute`，流水线入队需 `cicd_pipelines:execute`，脚本入队需 `cicd_script_jobs:execute`，`package` 需 `ai_skills:download`，接口文档 `push` 需 `project_docs:create`，`pull` / `export` 需 `project_docs:view`；开发文档对应 `project_dev_docs:*`；项目文档接口另要求项目 ACL。CI/CD 写/执行仍受角色 `data_scope` 约束。缺陷读路径（列表 / 详情 / 活动 / 评论 / 附件）需 `project_bugs:view`，创建 / 评论需 `project_bugs:create`，流转 / 上传附件需 `project_bugs:update`，均另受项目 ACL 约束；项目列表 `GET /projects` JWT 需 `project_projects:view`。
